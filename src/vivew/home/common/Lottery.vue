<template>
  <div id="app" >
    <main :class="['kd-body game game-c has-user-playing-info',{'cart-not-empty':Object.keys(select_data.betting_all).length}]">
   
        <div class="lottery-wrapper" >

          <!--隐藏的历史投注数据 @click="topData()"-->
                  ...
                  <br/>
                    ...
                    <ul class="nostyle kd-row prev-lottery-balls">
                      <li v-for="item1 in item['content']">
                        <span class="lottery-ball solid" :style="styleTool_2(true)">{{ item1 }}</span>
                      </li>
                      <li>
                        <span v-if="['2','6','7','8','9','12','13','28'].includes($route.params.id)" style="font-size:0.8rem;">{{sscMsg(item['content'])}}</span>
                      </li>
                    </ul>
                  <br/>
               </template>
          ...
          <div class="kd-container prev-lottery cqssc">
            ...
            <ul class="nostyle kd-row prev-lottery-balls">
                <li v-for="item in (data.history && data.history.content != 0 ? data.history.content.split(',') : ['正','在','开','奖'])"><span class="lottery-ball solid" :style="styleTool_2(true)">{{ item }}</span></li>
            </ul>
          </div>
        ...
        <section class="playing-method-section" v-for="(item,key) in data.basic" :id="key" v-if="'basic' in data && Object.keys(data.basic).length">
          ...
          <ul class="nostyle kd-row lottery-balls">
            <li v-for="(item1,key1) in item.items" :class="{'selected':(key in select_data.betting_all && key1 in select_data.betting_all[key])}" @click="betting(key,key1,(arrayIndexOf(select_data.alert_select,key) > -1 || arrayIndexOf(select_data.alert_select,key1) > -1 ? item1 : false))">
              <span :class="[/^\d+$/.test(item1.name)?'lottery-ball solid':'lottery-prop']" :style="styleTool_2(/^\d+$/.test(item1.name))">{{ item1.name }}</span>
        ...
        <form class="bet-form clearfix" v-show="Object.keys(select_data.betting_all).length" >
          <div class="kd-field">
            <label for="count_in_bet_form" class="kd-field-title">
              已选 <span>{{ zhushuSum }}</span> 注, 单注金额 <span>{{ amount?amount:0 }}</span> 元, 总共 <span>{{ amount*zhushuSum*(zh_data.expect_s.length ? zh_data.expect_s.length : 1) }}</span> 元
            </label>
      ...
      <transition name="bounce2">
        <div class="promptbox-wrapper" v-if="alert==1">
         <section class="promptbox prompt-service">
             <div class="prompt-head" style="letter-spacing:normal;" :style="styleTool(true)">发起总额为 {{ amount*zhushuSum*(zh_data.expect_s.length ? zh_data.expect_s.length : 1) }} 的合买</div>
...
<script>
  export default{
    ...
    methods: {
      sscMsg(val){
        let s,g,hs;
        g = (val[4]<=4?'小':'大') +  (val[4]%2==1?'单':'双');
        s = (val[3]<=4?'小':'大') +  (val[3]%2==1?'单':'双');
        let bit = [val[2],val[3],val[4]];
        if( Array.from(new Set(bit)).length == 3 ){
          hs = '组六';
        }else if(Array.from(new Set(bit)).length == 2){
          hs = '组三';
        }else{
          hs = '豹子';
        }
        return '十位('+ s +')个位('+ g +')后三('+ hs +')';
      },
      styleTool(value){
        let _data = {};
        if(value){
      ...
      hmCheck(){
        ...
        if(this.data.set.zg > 0 && parseInt(this.hm_data.buy_money / (sum / 100)) < this.data.set.zg){
          ...
          _this.hm_data.bd = Math.ceil((sum - this.hm_data.buy_money) / 100 * this.data.set.bd);
          return;
        }
      ...
      timeTool(){
        if(this.$route.path.indexOf('game-') == -1 || this.$route.path.indexOf('/game-l/') > -1 || this.$route.path.indexOf('/game-nc/') > -1 || this.$route.path.indexOf('/game-p/') > -1){
          return;
        }
        let _this = this
        ...
        _this.$getData({
          ...
          callback: function(res){
            if(res.code){
              ...
              _this.$store.state.title = res.data.name;
              _this.end_time = parseInt((new Date()).getTime())  + parseInt(res.data.number) * 1000;
              _this.timeTool();
              if(_this.zh_data.expect_s.length == 0){
                _this.zh_data.expect_s.push(_this.data.expect);
              }
              _this.loading = false;
              _this.down = 0;
            }else{
      ...
      getHistory(){
        ...
        _this.$getData({
          ...
          callback: function(res){
            if(res){
              if(res.expect - res.history.expect == 1){

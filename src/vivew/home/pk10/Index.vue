    ...
    <main :class="['kd-body game game-p game-id-pk10 has-user-playing-info',{'cart-not-empty':Object.keys(select_data.betting_all).length}]">
        ...
        <div class="lottery-wrapper"  @click="topData()">
          ...
          <transition name="bounce4">
              ...
              <div class="top_set_expect_div">
                ...
                <ul class="nostyle kd-row prev-lottery-balls">
                  <li v-for="item1 in item['content']">
                    <span :class="'lottery-ball solid pk10 ball' + parseInt(item1)">{{ item1 }}</span>
                  </li></ul><br/>
               </template>
              </div>
            </div>
            ...
            <div class="kd-container prev-lottery pk10" >
            
                <div class="prev-lottery-issue">
                  {{ (data.history ? data.history.expect : '...') }}
                </div>

                <ul class="nostyle kd-row prev-lottery-balls" >
                  <li v-if="'history' in data && 'content' in data.history && data.history.content == 0">
                    <span v-for="bit in ['正','在','开','奖']" class="lottery-ball solid " :style="'background:'+$store.state.basic.main_color+';'+'border:'+$store.state.basic.main_color">{{bit}}</span>
                  </li>
                  <li v-else v-for="(item,key) in (data.history ? data.history.content.split(',') : 0)">
                    <span :class="'lottery-ball solid pk10 ball' + parseInt(item)">{{ item }}</span>
                  </li>
                </ul>

            </div>

            <div class="kd-container pk10" >
...
<script>
  export default{
    ...
    methods: {
      ...
      timeTool(){
        if(this.$route.path.indexOf('/game-p/') == -1){
          return;
        }
        let _this = this
        ...
        if(cha < 1){
          ...
          _this.setTimeout_ids.push(setTimeout(() => {
            if(_this.data.expect && _this.data.history && 'history' in this.data && 'content' in this.data.history && _this.data.history.content == 0 && _this.down_loading){

<template>
  <div id="app">
    <main :class="['kd-body game game-p game-id-xync has-user-playing-info',{'cart-not-empty':Object.keys(select_data.betting_all).length}]">
        ...
        <div class="lottery-wrapper" @click="topData()">
          <!-- <transition name="bounce4">
              ...
              <div class="top_set_expect_div">
                ...
                <ul class="nostyle kd-row prev-lottery-balls">
                  <li v-for="item1 in item['content']">
                    <span :class="'lottery-ball solid xync ball' + parseInt(item1)">{{ item1 }}</span>
                  </li></ul><br/>
               </template>
              </div>
            </div>
            ...
            <div class="kd-container prev-lottery xync">
                <div class="prev-lottery-issue">{{ (data.history ? data.history.expect : '...') }}</div>
                <ul class="nostyle kd-row prev-lottery-balls">
                  <li v-for="(item,key) in (data.history && data.history.content != 0 ? data.history.content.split(',') : ['正','在','开','奖'])">
                    <span :class="'lottery-ball solid xync ball' + parseInt(item)">{{ item }}</span>
                  </li>
                </ul>
            </div>
        ...
        <section class="playing-method-section" v-for="(item,key) in data.basic" :id="key">
          ...
          <ul class="nostyle kd-row lottery-balls">
            <li v-for="(item1,key1) in item.items" :class="{'selected':(key in select_data.betting_all && key1 in select_data.betting_all[key])}" @click="betting(key,key1,(arrayIndexOf(select_data.alert_select,key) > -1 || arrayIndexOf(select_data.alert_select,key1) > -1 ? item1 : false))">
              <span :class="[/^\d+$/.test(item1.name)?'lottery-ball xync solid ball' + item1.name:'lottery-prop']">{{ item1.name }}</span>
...
<script>
  export default{
      ...
      select_data: {
        default: () => {
          return {
            alert_select: [],
            betting_all: {},
            single_select: { x: 'da',da: 'x',dan: 's',s: 'dan',wd: 'wx',wx: 'wd' }
          };
        }
      }
    },
    ...
    methods: {
      ...
      timeTool(){
        if(this.$route.path.indexOf('/game-nc/') == -1){
          return;
        }
        let _this = this
      ...
      target(key){
        let data = document.getElementById(key).offsetTop - 116;

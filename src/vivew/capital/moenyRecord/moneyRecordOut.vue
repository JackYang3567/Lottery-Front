    ...
    <template v-if="data_sel.length">
        <ul class="nostyle kd-container kd-list data-list" >
            <li class="item kd-box data" v-for="item in data_sel" :style="{ borderTop:'5px solid ' + $store.state.basic.main_color }">
                <div class="data-item"><span class="data-item-key">提款金额：</span><span class="data-item-value">￥{{item.money}}</span></div>
                <!-- <div class="data-item"><span class="data-item-key">交易帐号：</span><span class="data-item-value">{{item.pay_account}}</span></div> -->
                <div class="data-item"><span class="data-item-key">提款时间：</span><span class="data-item-value">{{item.create_time}}</span></div>
                <!---->
      ...
      methods:{
        ...
        queryData(){//数据请求
          ...
          _this.$getData({
            ...
            data:{ page:_this.page,where:JSON.stringify(_this.where),click:_this.click,code:4 },
            callback:function(res){

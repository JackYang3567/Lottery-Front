  ...
  <div id="app">
    ...
    <template v-if="data_sel.length">
        <ul class="nostyle kd-container kd-list data-list" >
            <li class="item kd-box data" :key="index" v-for="(item,key,index) in data_sel" :style="{ borderTop:'5px solid ' + $store.state.basic.main_color }">
                <div class="data-item"><span class="data-item-key">订单编号：</span><span class="data-item-value">{{item.order_id}}</span></div>
                <div class="data-item"><span class="data-item-key">充值方式：</span><span class="data-item-value">{{item.name}}</span></div>
                ...
                <div class="data-item"><span class="data-item-key">状    态：</span><span class="data-item-value">{{item.status}}</span></div>
                <div class="data-item" v-if="item['remarks']"><span class="data-item-key">备    注：</span><span class="data-item-value">{{item.remarks}}</span></div>
            </li>
        </ul>
        <div class="kd-pager">
    ...
    <transition name="bounce2">
                    ...
                    <div class="filter-tool-body">
                        <div class="kd-field">
                            ...
                            <div class="kd-textinput date">
                                <input v-model="where.order_id" placeholder="订单号将会优先查询" type="number">
                            </div>
                        </div>
                        <template  v-if="where.order_id == ''">
                        <div class="kd-field">
                            <label>起始时间</label>
                            <div class="kd-textinput date">
        ...
        <a href="javascript:;" class="filter-tool-toggle" @click="filter=true" v-if="!is_loadding" :style="{background: $store.state.basic.main_color }">过滤球</a>
    </div>
</template>

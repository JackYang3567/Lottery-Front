  ...
  <div id="app">
    <main class="kd-body">
        <form class="kd-form kd-container">
            ...
            <div class="kd-field" v-if="phoneOn == 1">
              <label for="user_name_in_register" class="kd-field-title required">手机号  <i class="text_msg" v-remind="form.phone"></i>  </label>
              <div class="kd-textinput textinput_mobile">
                  <input type="text" id="mobile_in_register" v-model.trim="form.phone" v-verify="form.phone" placeholder="请正确输入,保证帐号安全">
              </div>
            </div>
...
export default {
  ...
  methods: {
    ...
    submit(){
      let _this = this;
      // console.log(_this.form.username.length);
      // return;
      if(!this.$verify.check()){
        // _this.$toast('信息请填写完整');
        return false;
      }
      if(/[\u4e00-\u9fa5]/.test(_this.form.username)){
        if(_this.form.username.length < 2){
          _this.$toast('最少两个中文字符');
          return false;
        }
      }else if(_this.form.username.length < 4){
        _this.$toast('用户名过短,最少四个字符');
        return false;
      }

      if(_this.form.password != _this.form.to_password){
        _this.$toast('两次密码不一致');
        return false;
      }else if(!_this.read){
